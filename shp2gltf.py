...
class SHP2GLTFConverter:
    ...
    def read_shapefile(self, shp_path: str) -> gpd.GeoDataFrame:
        """
        读取SHP文件
        ...
        Returns:
            GeoDataFrame: 地理数据框
        """
        logger.info(f"正在读取SHP文件: {shp_path}")
        try:
            # 尝试不同的编码方式读取SHP文件
    ...
    def convert(self, shp_path: str, gltf_path: str, 
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='SHP转GLTF转换工具')
    parser.add_argument('input', help='输入SHP文件路径')
